import { ApiProperty } from '@nestjs/swagger';
import {
  ArrayNotEmpty,
  IsArray,
  IsNotEmpty,
  IsOptional,
  IsString,
  IsUrl,
  IsUUID,
  Matches,
} from 'class-validator';

export class CreateAppleOAuthProfileDto {
  @ApiProperty({
    description:
      'Unique URL-safe key used in callback URLs (e.g. /auth/<key>/apple/callback). Must be lowercase letters/numbers with dashes/underscores.',
    example: 'axis-ui-apple',
  })
  @IsString()
  @IsNotEmpty()
  @Matches(/^[a-z0-9_-]+$/, {
    message: 'key must match /^[a-z0-9_-]+$/',
  })
  key: string;

  @ApiProperty({ description: 'Human friendly name', example: 'Admin Apple OAuth' })
  @IsString()
  @IsNotEmpty()
  name: string;

  @ApiProperty({ description: 'Apple Services ID (clientID)', example: 'com.example.web' })
  @IsString()
  @IsNotEmpty()
  clientId: string;

  @ApiProperty({ description: 'Apple team ID', example: 'ABCD1234EF' })
  @IsString()
  @IsNotEmpty()
  teamId: string;

  @ApiProperty({ description: 'Apple key ID', example: '1A2B3C4D5E' })
  @IsString()
  @IsNotEmpty()
  keyId: string;

  @ApiProperty({
    description: 'Apple OAuth callback/redirect URI (must match Apple config)',
    example: 'https://api.example.com/auth/axis-ui-apple/apple/callback',
  })
  @IsString()
  @IsNotEmpty()
  @IsUrl({ require_tld: false })
  callbackUrl: string;

  @ApiProperty({
    description:
      'Role IDs allowed to use this profile. Descendant roles are also allowed.',
    example: ['4d2df2f7-7c14-4a5c-9ab4-6f76db0c7e5d'],
  })
  @IsArray()
  @ArrayNotEmpty()
  @IsUUID('4', { each: true })
  allowedRoleIds: string[];

  @ApiProperty({
    description:
      'Optional list of allowed email domains (e.g. ["x.com"]). If omitted/empty, any domain is allowed.',
    required: false,
    example: ['x.com'],
  })
  @IsArray()
  @IsString({ each: true })
  @IsOptional()
  allowedDomains?: string[];
}
